...
class ReportService:
    ...
    def generate_report_bytes(self) -> bytes:
        ...
        buffer = io.BytesIO()
        report_document = SimpleDocTemplate(
            buffer,
            pagesize=letter,
        ...
        if insights:
            for insight in insights:
                story.append(Paragraph(escape(str(insight.get("title", "Insight"))), body_style))
                description = insight.get("description", insight.get("body", ""))
                story.append(Paragraph(escape(str(description)), bullet_style))
                story.append(
                    Paragraph(
                        f"Severity: {escape(str(insight.get('severity', 'low')))} · "
                        f"Recommended timing: {escape(str(insight.get('timing', 'As advised')))}",
                        bullet_style,
                    )
                )
        else:
        ...
        if recommendations:
            for recommendation in recommendations:
                story.append(Paragraph(f"• {escape(recommendation)}", bullet_style))
        else:
        ...
        report_document.build(story)
        return buffer.getvalue()

    @staticmethod
    def _recommendations(insights: list[dict[str, Any]]) -> list[str]:
        """Extract unique maintenance actions from the diagnostic response."""
        ...
        for insight in insights:
            ...
            for action in actions:
                ...
                if isinstance(label, str) and label.strip() and label.strip() not in recommendations:
                    recommendations.append(label.strip())
